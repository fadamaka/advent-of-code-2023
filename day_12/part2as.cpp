...
long long workFunc(pair<string, vector<int>> p, vector<string> combos);
...
int main() {
    ...
    for (int i = 0; i < parsedLines.size(); i++) {
        ...
        vector<string> v1(combos2.begin(), combos2.begin() + (combos2.size() / threadCount));
        futureLongs.push_back(async(&workFunc, p2, v1));
        for (size_t j = 1; j < threadCount - 1; j++) {
            vector<string> v2(combos2.begin() + (combos2.size() / threadCount) * j, combos2.begin() + (combos2.size() / threadCount) * (j + 1));
            futureLongs.push_back(async(&workFunc, p2, v2));
        }

        vector<string> v8(combos2.begin() + (combos2.size() / threadCount) * (threadCount - 1), combos2.end());

        futureLongs.push_back(async(&workFunc, p2, v8));
        long long valid2 = 0;
