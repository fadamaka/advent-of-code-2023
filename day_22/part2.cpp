...
int aboveCounter(vector<Brick*> bricks, set<Brick*>& removed);
...
main() {
    ...
    for (auto&& i : bricks) {
        ...
        if (!distintFlag) {
            set<Brick*> newSet = {i};
            disCount += aboveCounter(i->above, newSet);
        }
        string str = distintFlag ? "TRUE" : "FALSE";
        set<Brick*> debugSet = {i};
        cout << *i
             << " dis:"
             << str
             << " a:"
...
int aboveCounter(vector<Brick*> bricks, set<Brick*>& removed) {
    ...
    for (auto&& i : bricks) {
        ...
        if (i->above.size() == 0 && below == 0) {
            n += 1;
            continue;
        }
        if (below == 0) {
            removed.insert(i);
            n += aboveCounter(i->above, removed) + 1;
        }
    }
    return n;
}
